...
def build_views(scored: Sequence[ScoredInstrument],
    ...
    ``reference_map`` pairs a single stock with the ETF it should be scored
    against, matching the ``activo_referencia`` column of CCI's basket sheet.
    A name with a reference produces a **relative** view; everything else
    produces an **absolute** one. Both legs of a relative view must be in the
    scored cross-section, otherwise the comparison would be against a name the
    model never ranked.
    """
...
def _rationale(row: ScoredInstrument, params: ViewParams, *,
               ...
               q: float = 0.0, conv: float = 0.0) -> str:
    """
